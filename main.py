...
from src.output.sap_mapper import map_bl_to_sap_payload, map_import_invoice_to_sap_payload
...
def main() -> None:
    ...
    if args.type == "bl":
        ...
        out_file = "output/import_invoice_extracted.json"
        write_json(out_file, normalized)
        print(f"[OK] extracted saved: {out_file}")

        if args.save_sap:
            sap_payload = map_import_invoice_to_sap_payload(inv_model)
            sap_file = "output/import_invoice_sap_payload.json"
            write_json(sap_file, sap_payload)
            print(f"[OK] sap payload saved: {sap_file}")
